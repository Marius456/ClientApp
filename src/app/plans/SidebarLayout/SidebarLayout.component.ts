...
export class SidebarLayoutComponent implements OnInit{
  ...
  CreateReview(review: Review) {
    ...
    this.ReviewService.addReview(review).subscribe(() => {
                                                      ...
                                                      sessionStorage.setItem('messagePlanDetail', "Review saved successfully");
                                                    });
    ...
    if (confirm("Are you sure to follow this plan?")) {   
      ...
      this.UserService.addBookmark(this.booking).subscribe(bookmark=>
                  this.UserService.addPlanTracker(bookmark).subscribe(() => {
                      this.UserService.checkPlanTracker(bookmark.bookmarkId).subscribe();   
                      window.location.reload();
                      sessionStorage.setItem('messagePlanDetail', "Bookmark saved successfully");
                  }))
    }
  }
}
